...
// Compute an encrypted value for string and store it in the keychain
+ (NSString*)createEncryptedValueForString:(NSString *)input
{
    NSUInteger *stringHash = [input hash];
    ...
    cipherBufferSize = SecKeyGetBlockSize(stringHash);
    cipherBuffer = malloc(cipherBufferSize);

    // Encrypt using the hash.
    status = SecKeyEncrypt(stringHash,
                           kSecPaddingPKCS1,
                           [input UTF8String],
                           (size_t) [input length],
                           cipherBuffer,
    ...
    NSData *encryptedData = [NSData dataWithBytes:cipherBuffer length:(size_t)[input length]];
    
    free(cipherBuffer);
    
    return (NSString*)encryptedData;
}
